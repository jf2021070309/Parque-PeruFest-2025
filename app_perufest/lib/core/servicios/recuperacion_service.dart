...
import '../utilidades/encriptacion_util.dart';
...
class RecuperacionService {
  ...
  Future<bool> enviarCodigoRecuperacion(String correo) async {
    try {
      ...
      await _supabase.from('codigos_recuperacion').insert({
        'correo': correo,
        'codigo': codigo,
      ...
      // Enviar email
      await _enviarEmail(correo, codigo, usuario['nombre'] ?? 'Usuario');
  ...
  Future<bool> verificarCodigo(String correo, String codigo) async {
    try {
      ...
      final expiraEn = DateTime.parse(resultado['expira_en']);
  ...
  Future<bool> cambiarContrasena(
    ...
    try {
      ...
      print('✅ Usuario encontrado: ${usuario['id']}');
